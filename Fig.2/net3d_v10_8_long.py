    ...
    class net_nodes():
        ...
        def get_degreesRS(nodes):
            ...
            for i in nodes.labels:
                if i not in nodes.deg_dict.keys():
                    nodes.deg_dict.setdefault(nodes.labels[i],0)
        ...
        def force_node_repel_brute(nodes):
            ...
            th = r0 + r0[:,newaxis]
#             th = 2*nodes.radius
            long = nodes.net.params['nodes']['long_rep']
            A = nodes.amplitude
            x = nodes.positions
            r = x - x[:,newaxis,:] #tf.expand_dims(x,1)
            rlen = vec_len(r)
            #fmat = A*r*tf.expand_dims((rlen/th)**(nodes.net.POWn-2)*tf.exp(-(rlen/th)**nodes.net.POWn),2)
            fmat = th[:,:,newaxis]**nodes.net.POW_SN *A*r*tf.expand_dims(long/(rlen/th+1e-5)**3+(rlen/th)**(nodes.net.POWn-2)*tf.exp(-(rlen/th)**nodes.net.POWn),2)
...
def check_convergence(self, tol = .01, win = 500, **kw):
    ...
    self.edgelen = sum(zip(*self.tv[-20:])[1])/20
