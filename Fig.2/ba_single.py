...
n3net.gnam = 'ba_n100_k6_relax%d-%d'%(relax,samp)
n3net.save(path='./', tv=0)

el = n3n.tf.reduce_sum(n3net.links.dp**2).eval()

net = jlo('./ba_n100_k6_relax%d-%d.json'%(relax,samp))
...
open('./ngln_ba_n100_k6.txt','a').write('%f\n'%network.m2_normalized)
open('./elastic_ba_n100_k6.txt','a').write('%f\n'%el)