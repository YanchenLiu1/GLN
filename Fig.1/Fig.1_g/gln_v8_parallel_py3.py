### deleted the network simplification part; do not neet Adj; node can have labels that are not integers; added cal_linking_num_array_new; added cal_m2_old_new; parallel; deleted radius and weights from edges; changed find_base_loops so that net doesn't need ['nodes']; fixed initialization of self.edges in the case where node labels are strings;
# note that adj_list cannot have multi-edges
###
...
def cal_norm(base_loops):
    new_norm = 0
    flag = 0
    base_loop1 = base_loops[0]
    base_loop2 = base_loops[1]
    if base_loop1 == base_loop2:
        flag=1
    for i in base_loop1.keys():
        for j in base_loop2.keys():
            if not set(base_loop1[i]['loop']).intersection(set(base_loop2[j]['loop'])): # no common nodes
                new_norm += 1
    if flag == 1:
        new_norm /= 2
...
def cal_linking_num_array(lp1, lp2):
    lp1 = array(lp1)
    lp2 = array(lp2)
#        X11 = lp1[1:,0]
#        X10 = lp1[:-1:,0]
...
def cal_linking_num_array_new(lp1, lp2):
    lp1 = array(lp1)
    lp2 = array(lp2)
...
#         self.new_norm = 0
    ...
    for i in base_loop1.keys():
        a = array(base_loop1[i]['points']).copy()
        c = array([a[k+1][0]-a[k][0] for k in range(len(a)-1)])

        for j in base_loop2.keys():
            if not set(base_loop1[i]['loop']).intersection(set(base_loop2[j]['loop'])): # no common nodes
                b = array(base_loop2[j]['points']).copy()
                ...
                if (xmin1<xmax2 and xmin2<xmax1) and (ymin1<ymax2 and ymin2<ymax1) and (zmin1<zmax2 and zmin2<zmax1):                      
                    ...
                    if prod((c > 0)+(c < 0)) == 0:
                        ...
                        d = array([b[k+1][0]-b[k][0] for k in range(len(b)-1)])
                        if prod((c > 0)+(c < 0)) == 0:
                            a1 = array(base_loop1[i]['points'])
                            o = where(c==0)[0] + 1
                            mask = ones(len(a1),dtype=bool)
                            mask[o] = False
                            a1 = a1[mask]
                            base_loop1[i]['points'] = [list(a1[u]) for u in range(len(a1))]
                            a = array(base_loop1[i]['points']).copy()
                            c = array([a[k+1][0]-a[k][0] for k in range(len(a)-1)])
                            if prod((c > 0)+(c < 0)) == 0:
                                a = rotate_x(a,pi*0.01354)
...
#                        d = array([b[k+1][0]-b[k][0] for k in range(len(b)-1)])
                    if prod((d > 0)+(d < 0)) == 0:
                        a2 = array(base_loop2[j]['points'])
                        o = where(d==0)[0] + 1
                        mask = ones(len(a2),dtype=bool)
                        mask[o] = False
                        a2 = a2[mask]
                        base_loop2[j]['points'] = [list(a2[u]) for u in range(len(a2))]
                        b = array(base_loop2[j]['points'])
#                        d = array([b[k+1][0]-b[k][0] for k in range(len(b)-1)])
#                            if prod((d > 0)+(d < 0)) == 0:
#                                b = rotate_x(b,pi*0.01354)
#                                b = rotate_y(b,pi*0.01354)
...
#                             c = array([a[k+1][0]-a[k][0] for k in range(len(a)-1)])
    ...
    #                                b = rotate_y(b,pi*0.01354)
                    ...
                    m2 =  cal_linking_num_array_new(a,b)
...
class simplify_v8():
    def __init__(self, adj_list, net, core=1):
        ...
        for edge in a:
            if self.edges[edge]['end_points'][0] <= self.edges[edge]['end_points'][1]:
                ...
                if '[%s, %s]'%(new_end_points[0],new_end_points[1]) not in self.edges:
                    self.edges['[%s, %s]'%(new_end_points[0],new_end_points[1])] = {0:{'end_points': new_end_points, 'points': self.edges[edge]['points'][::-1]}}
                else:
                    self.edges['[%s, %s]'%(new_end_points[0],new_end_points[1])][len(self.edges['[%s, %s]'%(new_end_points[0],new_end_points[1])])] = {'end_points': new_end_points, 'points': self.edges[edge]['points'][::-1]}
                del self.edges[edge]
...
#                 self.base_edges.add(tuple(edge[0]['end_points']))
            if len(edge) > 1: # multi-edge
                self.loop_set.add(frozenset(edge[0]['end_points']))
...
#                     root=gnodes.pop()
    ...
    #                 gnodes.remove(root)
                    a = list(gnodes)
#                     if n < len(a):
                    tmpn = int(len(gnodes)*random.rand())
#                         a = a[n+10:] + a[:n+10]
#                         a = a[tmpn:] + a[:tmpn]
                    root = a[tmpn]
#                     else:
...
#                             zused[z].add(z)
                            ...
                            while p not in pn:
                                ...
                                p=pred[p]
                            cycle.append(p)
                            if set(cycle) not in self.loop_set:
                                self.loop_set.add(frozenset(cycle))
...
#                     y = (k1*b2-k2*b1)/(k1-k2)
    ...
    def cal_m2_old_parallel(self):
        self.tangled_loops = {}
        self.m2 = 0
        self.gln_no_multi_windings = 0
        
        pool = ThreadPool(self.core) 
        results = pool.map(cal_m2_old_new, [[self.base_loop_secs[i], self.base_loop_secs[j]] for i,j in combinations_with_replacement(list(range(len(self.base_loop_secs))), 2)])
        self.m2 = sum(results)
        pool.close()
        pool.join()
...
#             new_num = len(self.base_loop)
    ...
    def cal_normalization_const_parallel(self):
        self.new_norm = 0
        pool = ThreadPool(self.core) 
        norms = pool.map(cal_norm, [[self.base_loop_secs[i], self.base_loop_secs[j]] for i,j in combinations_with_replacement(list(range(len(self.base_loop_secs))), 2)])
        self.new_norm = sum(norms)
        pool.close()
        pool.join()
                    
    def separate_base_loops(self):
        o = len(self.base_loop)
        num = int(sqrt(2*self.core))  # estimated number of sections
        interval = int(o/num) + 1
        self.base_loop_secs = [{i:j for i,j in self.base_loop.items() if i>= k*interval and i< (k+1)*interval} for k in range(num)]
#         self.base_loop_secs.append({i:j for i,j in self.base_loop.items() if i>= (num-1)*interval and i< o})
