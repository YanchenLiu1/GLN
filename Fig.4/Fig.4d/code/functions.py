...
def edge_list_2_adj(edge_list):
    N = max(max(zip(*edge_list)[0]),max(zip(*edge_list)[1]))+1
    Adj = zeros((N,N))
...
def gen_ER_m(n,m):
    Adj = zeros((n,n))
    e_p = list(combinations(range(n),2))
    e = ra.sample(e_p, m)
    return e
...
def E2A_weighted(E):
    N = max(max(zip(*E)[0]),max(zip(*E)[1]))+1
    Adj = zeros((N,N))
    for edge in E:
        if edge[0]!=edge[1]:
...
def add_line_perturbation_net_new(net,eps,seg=2): ## assume straight links
    for i in net['links']:
        ...
        #factor = random.normal(0,eps)
        intervel = (points[-1]-points[0])/seg
        center = array([(points[0] + 1.0*intervel + j*intervel).tolist() for j in range(seg-1)])
...
def get_binning_xy(data, num_bins = 15, is_pmf = True, log_binning = False, threshold = 0):
    
    # We need to define the support of our distribution
    lower_bound = min(list(data.keys()))
    upper_bound = max(list(data.keys()))
    ...
    for i in range(len(bin_edges)-1):
        ...
        for j in keys:
            if j < bin_edges[i+1]:
                bins[i].append(data[j])
                del data[j]
    
    p = [np.mean(bins[i]) for i in bins]
