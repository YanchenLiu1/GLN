...
class networkBase():
    def __init__(self, pts=[], edg=[],fixed=False, JSON=None, keep_paths=False, 
        ...
        self.init_vars()
        self.make_network()
        self.net_name = 'net-sim-' + ( os.path.split(JSON)[-1].strip('.json') if JSON else 'n%d-L%d'%(self.nodes.points.shape.as_list()[0], self.links.points.shape.as_list()[0]) ) \
        +'-th-r%.3g-segs%d-ka%.3g' %( self.links.thickness.mean()/self.nodes.radius,
                                     self.links.segs,self.params['links']['k']/self.params['links']['amplitude'])\
