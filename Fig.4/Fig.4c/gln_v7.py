    ...
    def __init__(self, Adj, net):
        ...
        for edge in a:
            if self.edges[edge]['end_points'][0] <= self.edges[edge]['end_points'][1]:
                if str(self.edges[edge]['end_points']) not in self.edges:
                    self.edges[str(self.edges[edge]['end_points'])] = {0:self.edges[edge]}
                else:
                    self.edges[str(self.edges[edge]['end_points'])][len(self.edges[str(self.edges[edge]['end_points'])])] = self.edges[edge]
                del self.edges[edge]
            else:
                new_end_points = self.edges[edge]['end_points'][::-1]
                if str(new_end_points) not in self.edges:
                    self.edges[str(new_end_points)] = {0:{'end_points': new_end_points, 'points': self.edges[edge]['points'][::-1], 'radius':self.edges[edge]['radius'], 'weight':self.edges[edge]['weight']}}
                else:
                    self.edges[str(new_end_points)][len(self.edges[str(new_end_points)])] = {'end_points': new_end_points, 'points': self.edges[edge]['points'][::-1], 'radius':self.edges[edge]['radius'], 'weight':self.edges[edge]['weight']}
                del self.edges[edge]
                    ...
                    for nbr in nonzero(self.Adj_new[self.idx_i[z],:])[0]:
                        ...
                        if nbr not in used:   # new node
                            ...
                            if set([z]) not in loop_set:
                                for i in self.edges['[%d, %d]'%(z,z)].keys():
                                    self.base_loop[len(self.base_loop)] = {'loop':[z, z], 'points':self.edges['[%d, %d]'%(z,z)][i]['points']}
                            ...
                            if set(cycle) not in loop_set:
                                ...
                                for i in range(len(cycle)-1):
                                    if cycle[i]<cycle[i+1]:
                                        points += self.edges['[%d, %d]'%(cycle[i],cycle[i+1])][0]['points'][1:]
                                    else:
                                        points += self.edges['[%d, %d]'%(cycle[i+1],cycle[i])][0]['points'][::-1][1:]
                                #points.append(points[0])
...
#         self.new_norm = 0
    ...
    #                                b = rotate_y(b,pi*0.01354)
                        ...
                        m2 =  self.cal_linking_num_array(a,b)
